  ...
  class TestResponder extends Actor {
    def receive = {
      case msg: CamelMessage =>msg.body match {
        case "fail" =>context.sender() ! akka.actor.Status.Failure(new AkkaCamelException(new Exception("failure")))
        case _ =>
...
//          val headers:Map[String,Any] = e.headers + ("test" -> "failure")
          context.sender() ! Status.Failure(
          new AkkaCamelException(e))
        }
      }
    }
  }
